...
class AvailabilityEans():
    ...
    def __init__(self, category_l1, category_l2) -> None:
        ...
        self.folder_path = os.path.join('data', category_l1, category_l2, 'availability')
        self.create_folders()
            ...
            async with aiohttp.ClientSession(headers=self.headers) as session:
                ...
                try:
                    async with session.get(url) as response:
                        ...
                        file_path = os.path.join(self.folder_path, f"{product}_availability.json")
                        with open(file_path, 'w', encoding='utf-8') as json_file:
                            json.dump(json_raw, json_file, indent=4, ensure_ascii=False)
                except Exception as e:
                    logger.error(f"Error fetching product {product} from {url}: {e}")

    async def scraping_availability(self, product_ids):
        logger.info(f"Scraping availability for {self.__category_l1}/{self.__category_l2} started.")
        print(f"Scraping availability for {self.__category_l1}/{self.__category_l2} started.")
        ...
        logger.info(f"Scraping availability for {self.__category_l1}/{self.__category_l2} finished.")
        print(f"Scraping availability for {self.__category_l1}/{self.__category_l2} finished.")